...
namespace Core.Services
{
    public class StationsService
    {
        ...
        public List<ProductsSales> GetProductsSales(Stations station, DateTime start, DateTime stop, string category) {
            ...
            SqlServerConnection conn = new SqlServerConnection();
            SqlDataReader dr = conn.SqlServerConnect("USE " + station.Prefix.Replace(".dbo.", "") + "; DECLARE @start DATE='" + start.Date + "', @stop DATE='" + stop.Date + "', @catg NVARCHAR(250)='" + category + "'; SELECT id_, Items, ISNULL(SUM(sd.qty*sd.price)/SUM(sd.qty),uPrice) sp, ISNULL(SUM(pd.qty*pd.price)/SUM(pd.qty),ISNULL((SELECT TOP(1) price FROM PurchasesDetails INNER JOIN Purchases ON PurNum=PurNo WHERE date_<=@stop AND id_=item_id ORDER BY date_ DESC),uPrice)) bp, ISNULL(SUM(CASE WHEN CAST(date_ AS DATE) BETWEEN @start AND @stop AND srcs_idnt=3 THEN qnty ELSE 0 END),0)- ISNULL(SUM(CASE WHEN CAST(date_ AS DATE) BETWEEN @start AND @stop AND srcs_idnt=12 THEN qnty ELSE 0 END),0) inn, ISNULL(SUM(CASE WHEN CAST(date_ AS DATE) BETWEEN @start AND @stop AND srcs_idnt=1 THEN qnty ELSE 0 END),0) sl, ISNULL(SUM(CASE WHEN CAST(date_ AS DATE) BETWEEN @start AND @stop AND srcs_idnt=11 THEN qnty ELSE 0 END),0) tr, uAvailable+ ISNULL(SUM(CASE WHEN pm_direction=0 AND CAST(date_ AS DATE)>@stop THEN qnty WHEN pm_direction=1 AND CAST(date_ AS DATE)>@stop THEN 0-qnty END),0) cl, ISNULL(SUM(sd.qty*sd.price),0) [income] FROM pProducts LEFT OUTER JOIN ProductsMovement pm ON id_=item_idnt AND CAST(date_ AS DATE)>=@start LEFT OUTER JOIN ProductsMovementSources ON srcs_idnt=pm_idnt LEFT OUTER JOIN SalesDetails sd ON sd.RcptNo_=tsid_idnt AND sd.item_=item_idnt AND srcs_idnt=1 AND CAST(date_ AS DATE) BETWEEN @start AND @stop LEFT OUTER JOIN PurchasesDetails pd ON pd.PurNum=tsid_idnt AND pd.item_id=item_idnt AND srcs_idnt=3 AND CAST(date_ AS DATE) BETWEEN @start AND @stop WHERE Category=@catg GROUP BY id_, Items, uPrice, uAvailable ORDER BY Items");
            if (dr.HasRows) {
                while (dr.Read()) {
